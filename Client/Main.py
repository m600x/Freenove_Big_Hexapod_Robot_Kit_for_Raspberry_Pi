...
class MyWindow(QMainWindow,Ui_client):
    def __init__(self):
        ...
        if hasattr(var, 'FILEPATH_WINDOW_ICON'):
            logging.debug("Set %s as window icon" % var.FILEPATH_WINDOW_ICON)
            self.setWindowIcon(QIcon(var.FILEPATH_WINDOW_ICON))
        if hasattr(var, 'FILEPATH_WINDOW_CAMERA'):
            logging.debug("Set %s as camera placeholder image" % var.FILEPATH_WINDOW_CAMERA)
            self.Video.setPixmap(QPixmap(var.FILEPATH_WINDOW_CAMERA))
        if hasattr(var, 'FILEPATH_IP'):
            try:
                file = open(var.FILEPATH_IP, 'r')
                self.lineEdit_IP_Adress.setText(str(file.readline()))
                logging.debug("Read IP %s from file" % self.lineEdit_IP_Adress.text())
                file.close()
            except FileNotFoundError:
                logging.error("Cannot read file %s defaulting to blank IP" % var.FILEPATH_IP)
        ...
        self.slider_head.setMaximum(var.HEAD_VERTICAL_MAX)
        self.slider_head.setSingleStep(var.HEAD_VERTICAL_STEP)
        self.slider_head.setValue(var.HEAD_VERTICAL_DEFAULT)
        self.slider_head.valueChanged.connect(self.headUpAndDown)

        # Head slider for horizontal movement
        self.slider_head_1.setMinimum(var.HEAD_HORIZONTAL_MIN)
        self.slider_head_1.setMaximum(var.HEAD_HORIZONTAL_MAX)
        self.slider_head_1.setSingleStep(var.HEAD_HORIZONTAL_STEP)
        self.slider_head_1.setValue(var.HEAD_HORIZONTAL_DEFAULT)
        self.slider_head_1.valueChanged.connect(self.headLeftAndRight)

        # Movement speed slider
        self.slider_speed.setMinimum(var.SPEED_MIN)
        self.slider_speed.setMaximum(var.SPEED_MAX)
        self.slider_speed.setSingleStep(var.SPEED_STEP)
        self.slider_speed.setValue(var.SPEED_DEFAULT)
        self.slider_speed.valueChanged.connect(self.speed)
        self.client.move_speed = str(self.slider_speed.value())

        # Roll slider
        self.slider_roll.setMinimum(var.ROLL_MIN)
        self.slider_roll.setMaximum(var.ROLL_MAX)
        self.slider_roll.setSingleStep(var.ROLL_STEP)
        self.slider_roll.setValue(var.ROLL_DEFAULT)
        self.slider_roll.valueChanged.connect(self.setRoll)

        # Z height slider
        self.slider_Z.setMinimum(var.Z_HEIGHT_MIN)
        self.slider_Z.setMaximum(var.Z_HEIGHT_MAX)
        self.slider_Z.setSingleStep(var.Z_HEIGHT_STEP)
        self.slider_Z.setValue(var.Z_HEIGHT_DEFAULT)
